...
def plotNetworkGraph(waveMatrix, locations,nodeNames, filename, thresholds, nodeSize,filterTo):
	...
	for thres in thresholds:
		...
		NA_cmap=plt.get_cmap('Greys')
		Others_cmap=plt.get_cmap('Blues')
		nodeColor=[]
		for name in nodeNames:
			...
			if(validateLocation(latitude,longitude)):
				...
				if getFront(name) in filterTo:
					nodeColor.append(NA_cmap(0.5))
				else:
					nodeColor.append(Others_cmap(0.5))
		...
		calCorrDistribution(waveMatrix,G_Amplitude,nodeNames,filename, filterTo)
		plt.close()
		...
		plt.savefig("{}/results_{}.png".format(filename.split('/')[0],filename.split('/')[1]), dpi = 300,bbox_inches='tight')
...
filterTo=pickle.load(open("NA/commonNACode.pickle","rb"))
filename="NA/mapCommonNA"
nodeSize=15
thresholds=[0.0]
