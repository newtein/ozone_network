...
def plotNetworkGraph(waveMatrix, locations,nodeNames, filename, thresholds, nodeSize,yearInput):
	...
	carb=[9,10,23,24,25,34,36,42,44,50,11, 6,41,53]
	noState = ['"Guam"','"Puerto Rico"','"Hawaii"','"Virgin Islands"','"Alaska"']
	noStateCode = [66,72,15,78,2]

	others = [int(i.split('-')[0]) for i in nodeNames if int(i.split('-')[0]) not in carb and int(i.split('-')[0]) not in noStateCode]
	print(list(set(others)))
	communities= [carb,list(set(others))]
	for thres in thresholds:
		...
		colorMaps=['Greens','Reds','Blues','Greys','Oranges']
		# so^>v<dph8
		...
		ax.text(-0.0003, 0.98, 'c)', transform=ax.transAxes, size=12,color='purple')
		...
		plt.savefig("JustCarbresults_"+str(thres)+"{}.png".format(yearInput), dpi = 300,bbox_inches='tight')
