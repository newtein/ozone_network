...
skip=[]
def plotNetworkGraph(plt,waveMatrix, locations,nodeNames, filename, thresholds, nodeSize,yearInput):
	cmap=plt.get_cmap("Purples")
	# carbA=[6,41,53,8,16,31,40,49]
	# carbB=[9,10,23,24,25,34,36,42,44,50,11,55,13]
	carbA=[6,41,53]
	...
	for thres in thresholds:
		...
		# so^>v<dph8
		if yearInput == 2017:
			plt.scatter(-3,0,s=5,label='Western CARB States',color = plt.get_cmap(colorMaps[0])(0.8))
			plt.scatter(-3,0,s=5,label='Others',color = plt.get_cmap(colorMaps[2])(0.8))
			plt.scatter(-3,0,s=5,label='Northeastern CARB States',color = plt.get_cmap(colorMaps[1])(0.8))

			plt.legend(loc='lower center',bbox_to_anchor=(0.5, -0.05),ncol=3,frameon=False,prop={'size': 6})
		plt.xlim(0,None)
		...
		#ax.text(-0.0005, 0.98, 'a)', transform=ax.transAxes, size=15,color='purple')
...
for yearInput in years:
	index = years.index(yearInput)
	row,col = rowcol[index]
	plt.subplot(gs[row,col[0]:col[1]])
	...
	nodeSize=15
	thresholds=[0.0]
	plt=plotNetworkGraph(plt,waveMatrix, locations,nodeNames, filename, thresholds, nodeSize,yearInput)

plt.tight_layout()
#plt.suptitle('US Ozone Network')
#plt.subplots_adjust(top=0.99,wspace=None, hspace=None)
plt.savefig("Carbresults_{}.png".format(yearInput), dpi = dpi,bbox_inches='tight')